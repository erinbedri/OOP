class PizzaDelivery:
    ...
    def make_order(self):
        self.ordered = True
        ingredients_to_print = ', '.join([f'{ingredient}: {quantity}' for ingredient, quantity in self.ingredients.items()])
        return f"You've ordered pizza {self.name} prepared with {ingredients_to_print} and the price will be {self.price}lv."
