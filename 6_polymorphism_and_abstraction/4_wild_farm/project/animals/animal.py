...
class Animal(ABC):
    ...
    def feed(self, food):
        if type(food) not in self.suitable_food:
...
class Bird(Animal, ABC):
    ...
    def __repr__(self):
        return f"{type(self).__name__} [{self.name}, {self.wing_size}, " \
...
class Mammal(Animal, ABC):
    ...
    def __repr__(self):
        return f"{type(self).__name__} [{self.name}, {self.weight}, " \
